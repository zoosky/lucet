...
mod lucetc_tests {
    ...
    fn empty() {
        ...
        let c = Compiler::new(&m, OptLevel::SpeedAndSize, &b, h, false).expect("compile empty");
    ...
    fn just_c() {
        ...
        let b = Bindings::empty();
        let h = HeapSettings::default();
        let c = Compiler::new(&m, OptLevel::SpeedAndSize, &b, h, false).expect("compile c");
        let mdata = c.module_data().unwrap();
        assert_eq!(mdata.import_functions().len(), 0, "import functions");
    ...
    fn just_d() {
        ...
        let h = HeapSettings::default();
        let c = Compiler::new(&m, OptLevel::SpeedAndSize, &b, h, false).expect("compile d");
        let mdata = c.module_data().unwrap();
    ...
    fn c_and_d() {
        ...
        let b = Bindings::empty();
        let h = HeapSettings::default();
        let c = Compiler::new(&m, OptLevel::SpeedAndSize, &b, h, false).expect("compile c & d");
        let mdata = c.module_data().unwrap();
        assert_eq!(mdata.import_functions().len(), 0, "import functions");
